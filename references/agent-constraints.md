...
- 若父级要求状态汇报、阶段汇报或终局摘要，开头优先使用 `当前已完成 / 上一步完成的是 / 下一步计划做的是 / 整体计划脉络图与当前位置` 四段结构，再补详细展开
- 若需要把长回复镜像到 `cli_console.md`，镜像内容必须与最终回复保持一致，并采用**覆盖写入**而不是追加堆积；如当前仓库提供 `redcap-cli-console-mirror.sh` 之类 helper，应优先使用统一 helper
