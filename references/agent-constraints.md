...
- 执行期若收到父级新增需求、纠偏、约束变更或优先级变化，必须先按父级控制面进入中插需求账本与重计划流程；不得直接完成最新插入项后，把它冒充为父任务整体完成
- 若父级将新要求拆成子任务，子 Agent 只能声明子任务交付完成；除非父级显式提供 parent receipt gate，否则不得声称父任务已完成
