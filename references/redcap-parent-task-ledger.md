...
| P2-16 | user inserted Codex update | Codex lifecycle hooks candidate 接线 | completed | P2 | 已接入 `.codex/config.toml` / `.codex/hooks.json`、SessionStart / PreToolUse / Stop wrapper、readiness/status/知识画像和 acceptance；Codex 从“入口导入 only”升级为“repo-owned candidate” | report: `2026-05-09-codex-lifecycle-hooks-candidate.md`；真实 trusted Codex marker E2E 前仍是 degraded，不等于 full host parity 或完整 reply-veto |
...
- 不可声明 P2-16 让 Codex 已经 full host parity；它只证明 repo-local candidate 配置、wrapper 和机器检查已存在，真实 SessionStart / Stop 触发仍需 trusted Codex live marker E2E。
