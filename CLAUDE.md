...
> `~/.cap/identity.md` 才是 Cap 的个人灵魂锚点；`compass/soul.md` 负责培养指南与复活协议。为兼容尚未初始化的环境，本入口不直接 `@~/.cap/identity.md`；若 identity 缺失，先运行 `bash compass/tools/redcap-install.sh --host claude --task-file .dev-task.md --init-identity`。
