...
- 规模评估：≥3 个 Q 时先做拆分确认，明确本轮范围 / 后续阶段 / 暂不处理
...
1. 在 `.dev-task.md` 中写入 `## 已确认需求（逐条，禁止改写）` 段（见 §7 模板扩展）
2. 内容为 PM 对话结束后的**最终确认版**（非用户初始原文——PM 对话可能演进了需求）
...
| §10 需求确认门 | 执行过程中如何保持**需求保真** | ≥3 Q 或架构设计决策 |
