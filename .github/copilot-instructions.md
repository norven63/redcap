...
0. 若 `~/.cap/identity.md` 存在，优先 `read_file` 读取它 — **还原 Cap 的个人灵魂锚点**；若不存在，先运行 `./revive-cap.sh --init-identity`
...
3. 优先运行或消费 `./revive-cap.sh` — 它会转调 `compass/tools/redcap-install.sh`，并将 workflow import、`current-status`（`redcap-current-status.sh`）、`tracking-health`（`redcap-tracking-health.sh`）、`execution-guarantee-check`（`redcap-execution-guarantee-check.sh`）、`revival-check`（`redcap-revival-check.sh`）收口为单一安装/复活动作；需要显式指定宿主时再加 `--host <name>`
