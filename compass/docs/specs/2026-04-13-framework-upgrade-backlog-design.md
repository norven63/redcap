...
- 当前焦点说明：正式发布 R1 的 Prism 证据保留拆分 dry-run 已完成：为 prism/tools、prism/README.md、prism/reports、prism/runs 建立证据保留分层、no-apply 清理预览边界、别名/回滚计划和机器验收；本项未删除证据、未移动目录、未关闭 blocker、未执行发布。
