...
- 当前焦点：`P4-2n 高价值经验发现与候选化发布硬门`
- 当前焦点说明：高价值经验发现与候选化已升级为正式发布前硬门；下一阶段若启动 formal release task，必须先通过 Evolution harvest signal gate，不能用候选池 strict 通过替代候选发现。
...
| 阶段 6：发布准备控制面 | 已完成 | P4-2l / P4-2m / P4-2n | 正式发布动作之前，发布路线、人工授权、历史资产物理清理硬门和高价值经验候选化硬门都已进入可审计控制面；当前仍不执行真实发布。 |
