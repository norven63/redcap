...
- 当前焦点：`P4-5 R1 Prism 证据保留 apply 预检`
- 当前焦点说明：正式发布 R1 的 Prism evidence apply 预检正在推进：把 P4-3e 的 dry-run 证据边界升级为 copy-first / alias-first apply 前置护栏；本项不移动、不删除、不清理 prism 证据，不关闭 blocker，不执行发布。
...
| 阶段 9：发布准备 Prism evidence apply 预检 | 进行中 | P4-5 | 正式发布动作之前，把 Prism evidence 未来 copy-first / alias-first apply 的前置护栏、证据保留策略、回滚门禁和验证计划做成机器可检查状态；当前仍不执行真实移动、删除、清理或发布。 |
...
| P4-5 R1 Prism 证据保留 apply 预检 | 发布前路线与授权边界 | 进行中 | P0 | 正在把 prism-layer-and-evidence 的 dry-run 证据边界升级为可机器检查的 copy-first / alias-first apply 前置护栏；本项不移动、不删除、不清理 prism 证据，不关闭 R1，不执行发布。 |
