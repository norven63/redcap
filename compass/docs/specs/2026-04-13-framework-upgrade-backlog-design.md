...
- 当前焦点说明：正式发布 R1 正在推进 internal-control-plane 物理拆分 dry-run：为 compass/references 的 package-visible control-plane 候选生成目标分层、别名/回滚计划和机器验收；本项不移动目录、不关闭 blocker、不执行发布。
...
| 阶段 6：发布准备控制面 | 已完成 | P4-2l / P4-2m / P4-2n / P4-2o / P4-2p / P4-2q / P4-2r / P4-2s | 正式发布动作之前，R1 延期根目录处置预检、三类 blocker 的专项预检，以及 internal-control-plane 的物理拆分 dry-run 正在进入可审计控制面；当前仍不执行真实发布。 |
...
| P4-2s R1 控制面物理拆分干跑清单 | 发布前路线与授权边界 | 进行中 | P0 | 正在把 internal-control-plane 的下一步物理拆分准备成可机器检查的 dry-run manifest、目标分层、别名/回滚计划和验收门禁；本项不关闭 R1、不执行发布、不移动 compass/references。 |
