...
- 当前焦点：`P4-2q R1 Prism 证据保留拆分预检`
- 当前焦点说明：正式发布 R1 已完成 prism-layer-and-evidence 的证据保留拆分预检；Prism 工具、报告、运行证据、包面边界、消费者矩阵和未来物理拆分/清理门禁已成为机器可查证据。
...
| 阶段 6：发布准备控制面 | 已完成 | P4-2l / P4-2m / P4-2n / P4-2o / P4-2p / P4-2q | 正式发布动作之前，发布路线、人工授权、历史资产物理清理硬门、高价值经验候选化硬门、R1 延期根目录处置预检、internal-control-plane 契约拆分预检和 prism-layer-and-evidence 证据保留拆分预检都已进入可审计控制面；当前仍不执行真实发布。 |
