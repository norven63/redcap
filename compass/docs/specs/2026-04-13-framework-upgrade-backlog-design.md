...
- 当前焦点：`P4-4 R1 控制面物理拆分 apply 预检`
- 当前焦点说明：正式发布 R1 的 control-plane apply 预检正在推进：把 P4-2s 的 dry-run 地图升级为 copy-first / alias-first apply 前置护栏；本项不复制、不移动、不删除旧 compass/references 锚点，不关闭 blocker，不执行发布。
...
| 阶段 8：发布准备控制面 apply 预检 | 进行中 | P4-4 | 正式发布动作之前，把控制面未来 copy-first / alias-first apply 的前置护栏、旧锚点保留策略、回滚门禁和验证计划做成机器可检查状态；当前仍不执行真实复制、移动、删除或发布。 |
...
| P4-4 R1 控制面物理拆分 apply 预检 | 发布前路线与授权边界 | 进行中 | P0 | 正在把 internal-control-plane 的 dry-run 地图升级为可机器检查的 copy-first / alias-first apply 前置护栏；本项不复制、不移动、不删除旧 compass/references 锚点，不关闭 R1，不执行发布。 |
