...
3. **变更前必须做经验回顾**：框架/治理/控制面变更前，先通过 `assets/knowledge/index.md` 与 `lessons.md` 的热点主题速览定位相关经验，不能因为 lessons 大就跳过已知失败模式。
...
11. **Codex 子 Agent 默认克制但不是禁用**：仅在确实能提效提质时开启，且 RedCap / Prism 主动拉起的 Codex-family 执行进程总数默认不超过 2（当前宿主也计入）；外部审查 / reviewer 选择统一按“模型能力画像 + 本地 CLI 稳定性”排序，不得静态压低 Copilot / Codex；显式 provider 冻结窗口以 `assets/references/prism-provider-policy.json` 为准，冻结期间不得调用对应 CLI。
...
| docs / knowledge / token 风险 | `CONTRIBUTING.md` §6、§7 的 docs/knowledge 边界、`assets/docs/index.yaml`、`assets/knowledge/index.md` |
...
| 自我进化 / 经验人格沉淀 | `compass/evolution/README.md`、`assets/references/evolution-candidate-schema.json`、`assets/references/evolution-grade-baseline.json` |
| 文件查阅 / 名词定位 | `assets/references/file-lookup-dictionary.md` |
| 共享知识 / 团队沉淀 | `assets/references/shared-knowledge-policy.json`、`templates/shared-knowledge/README.md` |
| skill 分发 / 多宿主入口 | `assets/references/skill-lifecycle-policy.json` |
