...
def fail(message: str) -> int:
    print(f"[redcap-token-risk-audit] {message}", file=sys.stderr)
    return 1
...
def git_ignored(root: pathlib.Path, rel_path: str) -> bool:
    return run_git(root, "check-ignore", "-q", rel_path).returncode == 0
...
def main() -> int:
    ...
    missing_mitigation = [
        (size, rel) for size, rel, mitigation in large_tracked if mitigation is None
    ]
    ...
    for rel in ENTRY_FILES:
        ...
        for token in FORBIDDEN_AUTO_IMPORTS:
            if any(raw.strip() == "@" + token for raw in text.splitlines()):
    ...
    if entry_failures:
        return fail("; ".join(entry_failures))
    ...
    for rel in required_scripts:
        if not (root / rel).is_file():
            return fail(f"missing mitigation script: {rel}")

    if ignored_issues:
        return fail("; ".join(ignored_issues))

    print("TOKEN_RISK_AUDIT_OK")
    print(
        f"tracked_large_files={len(large_tracked)} warn_bytes={TRACKED_WARN_BYTES} hard_bytes={TRACKED_HARD_BYTES}"
    )
    for size, rel, mitigation in sorted(large_tracked, reverse=True)[:15]:
        print(f"tracked\tbytes={size}\tpath={rel}\tmitigation={mitigation}")
    print(
        f"ignored_large_paths={len(ignored_large)} warn_bytes={IGNORED_WARN_BYTES} hard_bytes={IGNORED_HARD_BYTES} warn_files={IGNORED_WARN_FILES}"
    )
