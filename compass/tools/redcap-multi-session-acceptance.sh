...
assert_not_string_contains() {
    [[ "$1" != *"$2"* ]] || fail "expected '$1' not to contain '$2'"
}
...
run_layerb_closeout_runtime_uses_pending_baseline_case() {
    local host="codex"
    local current_head pending_baseline task_file report_path report_rel case_dir
    local on_complete_stub session_end_stub baseline_log output status pending_state

    log "case: layerb-closeout-runtime-uses-pending-baseline"

    current_head="$(git -C "$REDCAP_ROOT" rev-parse HEAD)"
    pending_baseline="$(git -C "$REDCAP_ROOT" rev-parse HEAD~1 2>/dev/null || printf '%s\n' "$current_head")"
    if [[ "$pending_baseline" == "$current_head" ]]; then
        log "skip: repository has no earlier commit to prove pending baseline fallback"
        return 0
    fi
...
## 零、先看懂当前局面
### 0.1 当前已完成
- 当前已完成：pending closure baseline fallback fixture
### 0.3 下一步计划做的是
- 下一步计划做的是：无
EOF
    write_layerb_closeout_task_fixture "$task_file" "$report_rel" "- [x] pending baseline 已准备"
    ...
    redcap_interop_write_pending_closure \
        ...
        "acceptance-seed" \
        "closeout-runtime" \
        "pending baseline fixture" \
        "$report_rel" \
        "$pending_baseline" \
        "$pending_baseline" \
        >/dev/null || fail "failed to seed pending closure baseline"
    pending_state="$(redcap_interop_pending_closure_file "$REDCAP_ROOT" "$task_file")"
    assert_exists "$pending_state"

    case_dir="$(mktemp -d "$ACCEPT_ROOT/closeout-runtime-pending-baseline.XXXXXX")"
    TEMP_PROJECTS+=("$case_dir")
...
EOF
    ...
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "pending baseline fixture unexpectedly completed"
...
run_legacy_asset_migration_worktree_rehearsal_case() {
    local fixture_root manifest result output bad_manifest stale_output status nogit_root nogit_manifest
    ...
    result="$ACCEPT_ROOT/legacy-asset-worktree-result.json"
    mkdir -p "$fixture_root"
    write_legacy_asset_rehearsal_fixture "$fixture_root" "$manifest"
    ...
    output="$(bash "$REDCAP_ROOT/compass/tools/redcap-legacy-asset-migration-worktree-rehearsal.sh" --root "$fixture_root" --manifest "$manifest" --result "$result" --write-result --check-result)"
    assert_string_contains "$output" "LEGACY_ASSET_MIGRATION_WORKTREE_REHEARSAL_OK"
    [[ -f "$result" ]] || fail "legacy asset worktree rehearsal should write result"
    [[ ! -e "$fixture_root/redcap-knowledge" ]] || fail "legacy asset worktree rehearsal must not create targets in fixture root"
    [[ -z "$(git -C "$fixture_root" status --porcelain=v1 --untracked-files=all)" ]] || fail "legacy asset worktree rehearsal must leave fixture git status clean"

    bad_manifest="$ACCEPT_ROOT/legacy-asset-worktree-stale-result.json"
    python3 - "$manifest" "$bad_manifest" <<'PY'
import json
import pathlib
import sys
...
PY
    set +e
    stale_output="$(bash "$REDCAP_ROOT/compass/tools/redcap-legacy-asset-migration-worktree-rehearsal.sh" --root "$fixture_root" --manifest "$bad_manifest" --result "$result" --check-result 2>&1)"
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "legacy asset worktree rehearsal should reject stale result"
    assert_string_contains "$stale_output" "result file stale or inconsistent"

    bad_manifest="$ACCEPT_ROOT/legacy-asset-worktree-move.json"
    python3 - "$manifest" "$bad_manifest" <<'PY'
import json
import pathlib
import sys
...
PY
    set +e
    stale_output="$(bash "$REDCAP_ROOT/compass/tools/redcap-legacy-asset-migration-worktree-rehearsal.sh" --root "$fixture_root" --manifest "$bad_manifest" 2>&1)"
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "legacy asset worktree rehearsal should reject move"
    assert_string_contains "$stale_output" "forbidden operation"

    bad_manifest="$ACCEPT_ROOT/legacy-asset-worktree-public-export.json"
    python3 - "$manifest" "$bad_manifest" <<'PY'
import json
import pathlib
import sys
...
PY
    ...
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "legacy asset worktree rehearsal should reject public-export"
    assert_string_contains "$stale_output" "forbidden operation"

    bad_manifest="$ACCEPT_ROOT/legacy-asset-worktree-traversal.json"
    python3 - "$manifest" "$bad_manifest" <<'PY'
import json
import pathlib
import sys
...
PY
    ...
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "legacy asset worktree rehearsal should reject target traversal"
    assert_string_contains "$stale_output" "safe repo-relative path"

    bad_manifest="$ACCEPT_ROOT/legacy-asset-worktree-public-target.json"
    python3 - "$manifest" "$bad_manifest" <<'PY'
import json
import pathlib
import sys
...
PY
    ...
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "legacy asset worktree rehearsal should reject public target"
    assert_string_contains "$stale_output" "public/shared repository"

    bad_manifest="$ACCEPT_ROOT/legacy-asset-worktree-duplicate-target.json"
    python3 - "$manifest" "$bad_manifest" <<'PY'
import json
import pathlib
import sys
...
PY
    ...
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "legacy asset worktree rehearsal should reject duplicate target"
    assert_string_contains "$stale_output" "duplicate copy target"

    bad_manifest="$ACCEPT_ROOT/legacy-asset-worktree-missing-guard.json"
    python3 - "$manifest" "$bad_manifest" <<'PY'
import json
import pathlib
import sys
...
PY
    ...
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "legacy asset worktree rehearsal should reject missing copy-first guard"
    assert_string_contains "$stale_output" "missing copy-first guard"

    mkdir -p "$fixture_root/redcap-knowledge/task-reports"
    printf 'existing\n' > "$fixture_root/redcap-knowledge/task-reports/a.md"
    set +e
    stale_output="$(bash "$REDCAP_ROOT/compass/tools/redcap-legacy-asset-migration-worktree-rehearsal.sh" --root "$fixture_root" --manifest "$manifest" 2>&1)"
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "legacy asset worktree rehearsal should reject existing main-tree target"
    assert_string_contains "$stale_output" "main-tree target already exists"
    rm -rf "$fixture_root/redcap-knowledge"

    nogit_root="$ACCEPT_ROOT/legacy-asset-worktree-nogit"
    nogit_manifest="$ACCEPT_ROOT/legacy-asset-worktree-nogit.json"
    mkdir -p "$nogit_root"
    write_legacy_asset_rehearsal_fixture "$nogit_root" "$nogit_manifest"
    set +e
    stale_output="$(bash "$REDCAP_ROOT/compass/tools/redcap-legacy-asset-migration-worktree-rehearsal.sh" --root "$nogit_root" --manifest "$nogit_manifest" 2>&1)"
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "legacy asset worktree rehearsal should reject non-git roots"
...
run_r1_control_plane_physical_apply_preflight_check_case() {
    ...
    log "case: r1-control-plane-physical-apply-preflight-check"

    output="$(bash "$REDCAP_ROOT/compass/tools/redcap-r1-control-plane-physical-apply-preflight-check.sh")"
    assert_string_contains "$output" "R1_CONTROL_PLANE_PHYSICAL_APPLY_PREFLIGHT_OK"
    assert_string_contains "$output" "release_blocker_status=still-blocking"
    assert_contains "$REDCAP_ROOT/compass/tools/redcap-spec-check.sh" 'R1 control-plane physical apply preflight check missing'

    bad_manifest="$ACCEPT_ROOT/r1-control-plane-physical-apply-missing-source.json"
    python3 - "$REDCAP_ROOT/references/r1-control-plane-physical-apply-preflight.json" "$bad_manifest" <<'PY'
import json
import pathlib
import sys
src, dst = map(pathlib.Path, sys.argv[1:3])
...
PY
    set +e
    stale_output="$(python3 "$REDCAP_ROOT/compass/tools/redcap-r1-control-plane-physical-apply-preflight-check.py" --manifest "$bad_manifest" 2>&1)"
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "R1 control-plane physical apply checker should require source_truth"
    assert_string_contains "$stale_output" "source_truth"

    bad_manifest="$ACCEPT_ROOT/r1-control-plane-physical-apply-delete-allowed.json"
    python3 - "$REDCAP_ROOT/references/r1-control-plane-physical-apply-preflight.json" "$bad_manifest" <<'PY'
import json
import pathlib
import sys
src, dst = map(pathlib.Path, sys.argv[1:3])
payload = json.loads(src.read_text(encoding="utf-8"))
payload["operation_policy"]["destructive_operations_allowed"] = True
payload["operation_policy"]["forbidden_operations"] = [item for item in payload["operation_policy"]["forbidden_operations"] if item != "delete"]
dst.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
PY
    set +e
    stale_output="$(python3 "$REDCAP_ROOT/compass/tools/redcap-r1-control-plane-physical-apply-preflight-check.py" --manifest "$bad_manifest" 2>&1)"
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "R1 control-plane physical apply checker should reject destructive operations"
    assert_string_contains "$stale_output" "destructive_operations_allowed"

    bad_manifest="$ACCEPT_ROOT/r1-control-plane-physical-apply-stale-source.json"
    python3 - "$REDCAP_ROOT/references/r1-control-plane-physical-apply-preflight.json" "$bad_manifest" <<'PY'
import json
import pathlib
import sys
...
PY
    ...
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "R1 control-plane physical apply checker should reject stale source hash"
    assert_string_contains "$stale_output" "source_truth.sha256"

    bad_manifest="$ACCEPT_ROOT/r1-control-plane-physical-apply-resolved.json"
    python3 - "$REDCAP_ROOT/references/r1-control-plane-physical-apply-preflight.json" "$bad_manifest" <<'PY'
import json
import pathlib
import sys
...
PY
    ...
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "R1 control-plane physical apply checker should reject resolved blocker claims"
    assert_string_contains "$stale_output" "claim_boundary.release_blocker_resolved"
}
...
run_r1_prism_report_archive_copy_first_preflight_check_case() {
    ...
    log "case: r1-prism-report-archive-copy-first-preflight-check"

    output="$(bash "$REDCAP_ROOT/compass/tools/redcap-r1-prism-report-archive-copy-first-preflight-check.sh")"
    assert_string_contains "$output" "R1_PRISM_REPORT_ARCHIVE_COPY_FIRST_PREFLIGHT_OK"
    assert_string_contains "$output" "reports="
    assert_string_contains "$output" "index="
    assert_string_contains "$output" "runs="
    assert_string_contains "$output" "release_blocker_status=still-blocking"
    assert_contains "$REDCAP_ROOT/compass/tools/redcap-spec-check.sh" 'R1 Prism report archive copy-first preflight check missing'

    bad_preflight="$ACCEPT_ROOT/r1-prism-report-archive-copy-first-preflight-missing-source.json"
    python3 - "$REDCAP_ROOT/references/r1-prism-report-archive-copy-first-preflight.json" "$bad_preflight" <<'PY'
import json
import pathlib
import sys
src, dst = map(pathlib.Path, sys.argv[1:3])
...
PY
    set +e
    stale_output="$(python3 "$REDCAP_ROOT/compass/tools/redcap-r1-prism-report-archive-copy-first-preflight-check.py" --preflight "$bad_preflight" 2>&1)"
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "R1 Prism report archive checker should require source_truth"
    assert_string_contains "$stale_output" "source_truth"

    bad_preflight="$ACCEPT_ROOT/r1-prism-report-archive-copy-first-preflight-stale-source.json"
    python3 - "$REDCAP_ROOT/references/r1-prism-report-archive-copy-first-preflight.json" "$bad_preflight" <<'PY'
import json
import pathlib
import sys
src, dst = map(pathlib.Path, sys.argv[1:3])
payload = json.loads(src.read_text(encoding="utf-8"))
payload["source_truth"]["route_decision_sha256"] = "stale"
dst.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
PY
    set +e
    stale_output="$(python3 "$REDCAP_ROOT/compass/tools/redcap-r1-prism-report-archive-copy-first-preflight-check.py" --preflight "$bad_preflight" 2>&1)"
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "R1 Prism report archive checker should reject stale route decision hash"
    assert_string_contains "$stale_output" "source_truth.route_decision_sha256"

    bad_preflight="$ACCEPT_ROOT/r1-prism-report-archive-copy-first-preflight-cleanup-allowed.json"
    python3 - "$REDCAP_ROOT/references/r1-prism-report-archive-copy-first-preflight.json" "$bad_preflight" <<'PY'
import json
import pathlib
import sys
src, dst = map(pathlib.Path, sys.argv[1:3])
payload = json.loads(src.read_text(encoding="utf-8"))
payload["claim_boundary"]["raw_run_evidence_cleaned"] = True
...
PY
    ...
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "R1 Prism report archive checker should reject cleanup claims"
    assert_string_contains "$stale_output" "claim_boundary.raw_run_evidence_cleaned"

    bad_preflight="$ACCEPT_ROOT/r1-prism-report-archive-copy-first-preflight-resolved.json"
    python3 - "$REDCAP_ROOT/references/r1-prism-report-archive-copy-first-preflight.json" "$bad_preflight" <<'PY'
import json
import pathlib
import sys
...
PY
    ...
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "R1 Prism report archive checker should reject resolved blocker claims"
    assert_string_contains "$stale_output" "claim_boundary.release_blocker_resolved"
}
...
run_r1_prism_report_archive_copy_first_plan_check_case() {
    ...
    log "case: r1-prism-report-archive-copy-first-plan-check"

    output="$(bash "$REDCAP_ROOT/compass/tools/redcap-r1-prism-report-archive-copy-first-plan-check.sh")"
    assert_string_contains "$output" "R1_PRISM_REPORT_ARCHIVE_COPY_FIRST_PLAN_OK"
    assert_string_contains "$output" "mappings="
    assert_string_contains "$output" "release_blocker_status=still-blocking"
    assert_string_contains "$output" "release_blocker_status=still-blocking"
    assert_contains "$REDCAP_ROOT/compass/tools/redcap-spec-check.sh" 'R1 Prism report archive copy-first plan check missing'

    bad_plan="$ACCEPT_ROOT/r1-prism-report-archive-copy-first-plan-missing-source.json"
...
PY
    set +e
    stale_output="$(python3 "$REDCAP_ROOT/compass/tools/redcap-r1-prism-report-archive-copy-first-plan-check.py" --plan "$bad_plan" 2>&1)"
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "R1 Prism report archive plan checker should require source_truth"
    assert_string_contains "$stale_output" "source_truth"

    bad_plan="$ACCEPT_ROOT/r1-prism-report-archive-copy-first-plan-stale-source.json"
    python3 - "$REDCAP_ROOT/references/r1-prism-report-archive-copy-first-plan.json" "$bad_plan" <<'PY'
import json
import pathlib
import sys
src, dst = map(pathlib.Path, sys.argv[1:3])
payload = json.loads(src.read_text(encoding="utf-8"))
payload["source_truth"]["preflight_sha256"] = "stale"
dst.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
PY
    set +e
    stale_output="$(python3 "$REDCAP_ROOT/compass/tools/redcap-r1-prism-report-archive-copy-first-plan-check.py" --plan "$bad_plan" 2>&1)"
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "R1 Prism report archive plan checker should reject stale source hash"
    assert_string_contains "$stale_output" "source_truth.preflight_sha256"

    bad_plan="$ACCEPT_ROOT/r1-prism-report-archive-copy-first-plan-missing-mapping.json"
    python3 - "$REDCAP_ROOT/references/r1-prism-report-archive-copy-first-plan.json" "$bad_plan" <<'PY'
import json
import pathlib
import sys
src, dst = map(pathlib.Path, sys.argv[1:3])
payload = json.loads(src.read_text(encoding="utf-8"))
payload["archive_plan"]["mappings"] = payload["archive_plan"]["mappings"][:-1]
dst.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
PY
    set +e
    stale_output="$(python3 "$REDCAP_ROOT/compass/tools/redcap-r1-prism-report-archive-copy-first-plan-check.py" --plan "$bad_plan" 2>&1)"
...
src, dst = map(pathlib.Path, sys.argv[1:3])
payload = json.loads(src.read_text(encoding="utf-8"))
payload["claim_boundary"]["copy_performed_now"] = True
payload["archive_plan"]["mappings"][0]["copy_now"] = True
dst.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
PY
    set +e
    stale_output="$(python3 "$REDCAP_ROOT/compass/tools/redcap-r1-prism-report-archive-copy-first-plan-check.py" --plan "$bad_plan" 2>&1)"
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "R1 Prism report archive plan checker should reject copy-now claims"
    assert_string_contains "$stale_output" "claim_boundary.copy_performed_now"

    bad_plan="$ACCEPT_ROOT/r1-prism-report-archive-copy-first-plan-delete-last.json"
    python3 - "$REDCAP_ROOT/references/r1-prism-report-archive-copy-first-plan.json" "$bad_plan" <<'PY'
import json
import pathlib
import sys
src, dst = map(pathlib.Path, sys.argv[1:3])
payload = json.loads(src.read_text(encoding="utf-8"))
payload["claim_boundary"]["delete_last_performed_now"] = True
payload["archive_plan"]["mappings"][0]["delete_old_now"] = True
dst.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
PY
    set +e
    stale_output="$(python3 "$REDCAP_ROOT/compass/tools/redcap-r1-prism-report-archive-copy-first-plan-check.py" --plan "$bad_plan" 2>&1)"
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "R1 Prism report archive plan checker should reject delete-last claims"
    assert_string_contains "$stale_output" "claim_boundary.delete_last_performed_now"

    bad_plan="$ACCEPT_ROOT/r1-prism-report-archive-copy-first-plan-raw-evidence.json"
    python3 - "$REDCAP_ROOT/references/r1-prism-report-archive-copy-first-plan.json" "$bad_plan" <<'PY'
import json
import pathlib
import sys
...
PY
    ...
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "R1 Prism report archive plan checker should reject raw evidence claims"
    assert_string_contains "$stale_output" "claim_boundary.raw_run_evidence_touched"

    bad_plan="$ACCEPT_ROOT/r1-prism-report-archive-copy-first-plan-destructive.json"
    python3 - "$REDCAP_ROOT/references/r1-prism-report-archive-copy-first-plan.json" "$bad_plan" <<'PY'
import json
import pathlib
import sys
...
PY
    ...
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "R1 Prism report archive plan checker should reject destructive operation claims"
    assert_string_contains "$stale_output" "operation_policy.destructive_operations_allowed"

    local temp_archive_report="$REDCAP_ROOT/private-archive/prism-reports/__acceptance-temporary.md"
    mkdir -p "$(dirname "$temp_archive_report")"
    printf 'temporary acceptance copy must be rejected\n' >"$temp_archive_report"
    set +e
    stale_output="$(bash "$REDCAP_ROOT/compass/tools/redcap-r1-prism-report-archive-copy-first-plan-check.sh" 2>&1)"
    status=$?
...
PY
    ...
    status=$?
    set -e
    [[ "$status" -ne 0 ]] || fail "R1 Prism report archive plan checker should reject resolved blocker claims"
    assert_string_contains "$stale_output" "claim_boundary.release_blocker_resolved"
}
...
run_cli_product_surface_check_case() {
    local output workspace trace_output debug_output typo_output status

    log "case: cli-product-surface-check"

    output="$(bash "$REDCAP_ROOT/compass/tools/redcap-cli-product-surface-check.sh")"
    assert_string_contains "$output" "CLI_PRODUCT_SURFACE_OK"

    workspace="$ACCEPT_ROOT/cli-product-surface-workspace"
    mkdir -p "$workspace"
    cat >"$workspace/.dev-task.md" <<'EOF'
# 当前任务：cli product surface acceptance

## 控制面元数据（机器校验）
task_id: acceptance-cli-product-surface
active_slice: acceptance-cli-product-surface
...
EOF
    ...
    debug_output="$(cd "$workspace" && "$REDCAP_ROOT/bin/redcap" debug --json)"
    assert_string_contains "$debug_output" "\"task_id\": \"acceptance-cli-product-surface\""
    assert_string_contains "$debug_output" "\"boundary_mode\": \"external-workspace\""
    assert_not_string_contains "$debug_output" "/Users/"
    assert_not_string_contains "$debug_output" "/home/"
    assert_not_string_contains "$debug_output" "identity_file"
    ...
    set +e
    typo_output="$(cd "$workspace" && "$REDCAP_ROOT/bin/redcap" docter 2>&1)"
    status=$?
    set -e
...
EOF
        ...
        cat >"$repo/compass/tools/redcap-cli-product-surface-check.sh" <<EOF
#!/usr/bin/env bash
if [[ "$failing_gate" == "cli-product-surface" ]]; then
    echo "fixture CLI product surface failure" >&2
    exit 37
fi
exit 0
EOF
...
case "$COMMAND" in
    ...
    r1-prism-package-visible-support-copy-first-apply-check)
        run_r1_prism_package_visible_support_copy_first_apply_check_case
        ;;
    r1-prism-report-archive-copy-first-preflight-check)
        run_r1_prism_report_archive_copy_first_preflight_check_case
        ;;
