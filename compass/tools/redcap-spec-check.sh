...
R1_PRISM_EVIDENCE_RETENTION_APPLY_PREFLIGHT_CHECK="$REDCAP_ROOT/compass/tools/redcap-r1-prism-evidence-retention-apply-preflight-check.sh"
if [[ ! -f "$R1_PRISM_EVIDENCE_RETENTION_APPLY_PREFLIGHT_CHECK" ]]; then
    echo "[redcap-spec-check] R1 Prism evidence retention apply preflight check missing" >&2
    exit 1
fi
if ! bash "$R1_PRISM_EVIDENCE_RETENTION_APPLY_PREFLIGHT_CHECK" >/dev/null; then
    echo "[redcap-spec-check] R1 Prism evidence retention apply preflight check failed" >&2
    exit 1
fi
...
R1_PRISM_REPORT_ARCHIVE_COPY_FIRST_PREFLIGHT_CHECK="$REDCAP_ROOT/compass/tools/redcap-r1-prism-report-archive-copy-first-preflight-check.sh"
if [[ ! -f "$R1_PRISM_REPORT_ARCHIVE_COPY_FIRST_PREFLIGHT_CHECK" ]]; then
    echo "[redcap-spec-check] R1 Prism report archive copy-first preflight check missing" >&2
    exit 1
fi
if ! bash "$R1_PRISM_REPORT_ARCHIVE_COPY_FIRST_PREFLIGHT_CHECK" >/dev/null; then
    echo "[redcap-spec-check] R1 Prism report archive copy-first preflight check failed" >&2
    exit 1
fi

R1_PRISM_REPORT_ARCHIVE_COPY_FIRST_PLAN_CHECK="$REDCAP_ROOT/compass/tools/redcap-r1-prism-report-archive-copy-first-plan-check.sh"
if [[ ! -f "$R1_PRISM_REPORT_ARCHIVE_COPY_FIRST_PLAN_CHECK" ]]; then
    echo "[redcap-spec-check] R1 Prism report archive copy-first plan check missing" >&2
    exit 1
fi
if ! bash "$R1_PRISM_REPORT_ARCHIVE_COPY_FIRST_PLAN_CHECK" >/dev/null; then
    echo "[redcap-spec-check] R1 Prism report archive copy-first plan check failed" >&2
    exit 1
fi
...
R1_PRISM_REPORT_ARCHIVE_CHURN_FREEZE_GUARD_CHECK="$REDCAP_ROOT/compass/tools/redcap-r1-prism-report-archive-churn-freeze-guard-check.sh"
if [[ ! -f "$R1_PRISM_REPORT_ARCHIVE_CHURN_FREEZE_GUARD_CHECK" ]]; then
    echo "[redcap-spec-check] R1 Prism report archive churn/freeze guard check missing" >&2
    exit 1
fi
if ! bash "$R1_PRISM_REPORT_ARCHIVE_CHURN_FREEZE_GUARD_CHECK" >/dev/null; then
    echo "[redcap-spec-check] R1 Prism report archive churn/freeze guard check failed" >&2
    exit 1
fi
...
R1_CONTROL_PLANE_INTERNAL_MAINTAINER_FACADE_COPY_FIRST_APPLY_CHECK="$REDCAP_ROOT/compass/tools/redcap-r1-control-plane-internal-maintainer-facade-copy-first-apply-check.sh"
if [[ -f "$REDCAP_ROOT/references/r1-control-plane-internal-maintainer-facade-copy-first-apply.json" ]]; then
    if [[ ! -f "$R1_CONTROL_PLANE_INTERNAL_MAINTAINER_FACADE_COPY_FIRST_APPLY_CHECK" ]]; then
        echo "[redcap-spec-check] R1 control-plane internal maintainer facade copy-first apply check missing" >&2
        exit 1
    fi
    if ! bash "$R1_CONTROL_PLANE_INTERNAL_MAINTAINER_FACADE_COPY_FIRST_APPLY_CHECK" >/dev/null; then
        echo "[redcap-spec-check] R1 control-plane internal maintainer facade copy-first apply check failed" >&2
        exit 1
    fi
fi
...
R1_CONTROL_PLANE_INTERNAL_MAINTAINER_FACADE_BATCH_2_COPY_FIRST_APPLY_CHECK="$REDCAP_ROOT/compass/tools/redcap-r1-control-plane-internal-maintainer-facade-batch-2-copy-first-apply-check.sh"
if [[ -f "$REDCAP_ROOT/references/r1-control-plane-internal-maintainer-facade-batch-2-copy-first-apply.json" ]]; then
    if [[ ! -f "$R1_CONTROL_PLANE_INTERNAL_MAINTAINER_FACADE_BATCH_2_COPY_FIRST_APPLY_CHECK" ]]; then
        echo "[redcap-spec-check] R1 control-plane internal maintainer facade batch-2 copy-first apply check missing" >&2
        exit 1
    fi
    if ! bash "$R1_CONTROL_PLANE_INTERNAL_MAINTAINER_FACADE_BATCH_2_COPY_FIRST_APPLY_CHECK" >/dev/null; then
        echo "[redcap-spec-check] R1 control-plane internal maintainer facade batch-2 copy-first apply check failed" >&2
        exit 1
    fi
fi

R1_NEXT_SAFE_SLICE_AFTER_INTERNAL_MAINTAINER_FACADE_BATCH_2_CHECK="$REDCAP_ROOT/compass/tools/redcap-r1-next-safe-slice-after-internal-maintainer-facade-batch-2-check.sh"
if [[ -f "$REDCAP_ROOT/references/r1-next-safe-slice-after-internal-maintainer-facade-batch-2.json" ]]; then
    if [[ ! -f "$R1_NEXT_SAFE_SLICE_AFTER_INTERNAL_MAINTAINER_FACADE_BATCH_2_CHECK" ]]; then
        echo "[redcap-spec-check] R1 next safe slice after internal maintainer facade batch-2 check missing" >&2
        exit 1
    fi
    if ! bash "$R1_NEXT_SAFE_SLICE_AFTER_INTERNAL_MAINTAINER_FACADE_BATCH_2_CHECK" >/dev/null; then
        echo "[redcap-spec-check] R1 next safe slice after internal maintainer facade batch-2 check failed" >&2
        exit 1
    fi
fi
...
R1_NEXT_SAFE_SLICE_AFTER_CONTROL_PLANE_CONTRACT_MIRROR_PREFLIGHT_CHECK="$REDCAP_ROOT/compass/tools/redcap-r1-next-safe-slice-after-control-plane-contract-mirror-preflight-check.sh"
if [[ -f "$REDCAP_ROOT/references/r1-next-safe-slice-after-control-plane-contract-mirror-preflight.json" ]]; then
    if [[ ! -f "$R1_NEXT_SAFE_SLICE_AFTER_CONTROL_PLANE_CONTRACT_MIRROR_PREFLIGHT_CHECK" ]]; then
        echo "[redcap-spec-check] R1 next safe slice after control-plane contract mirror preflight check missing" >&2
        exit 1
    fi
    if ! bash "$R1_NEXT_SAFE_SLICE_AFTER_CONTROL_PLANE_CONTRACT_MIRROR_PREFLIGHT_CHECK" >/dev/null; then
        echo "[redcap-spec-check] R1 next safe slice after control-plane contract mirror preflight check failed" >&2
        exit 1
    fi
fi

R1_CONTRACT_MIRROR_APPLY_PREFLIGHT_SUBSET_CHECK="$REDCAP_ROOT/compass/tools/redcap-r1-contract-mirror-apply-preflight-subset-check.sh"
if [[ -f "$REDCAP_ROOT/references/r1-contract-mirror-apply-preflight-subset.json" ]]; then
    if [[ ! -f "$R1_CONTRACT_MIRROR_APPLY_PREFLIGHT_SUBSET_CHECK" ]]; then
        echo "[redcap-spec-check] R1 contract mirror apply preflight subset check missing" >&2
        exit 1
    fi
    if ! bash "$R1_CONTRACT_MIRROR_APPLY_PREFLIGHT_SUBSET_CHECK" >/dev/null; then
        echo "[redcap-spec-check] R1 contract mirror apply preflight subset check failed" >&2
        exit 1
    fi
fi
...
if ! bash "$R1_LAYERA_PRODUCT_BOUNDARY_CHECK" >/dev/null; then
    echo "[redcap-spec-check] R1 Layer A product boundary check failed" >&2
    exit 1
fi
