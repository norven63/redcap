#!/usr/bin/env python3
# 用途：正式发布 R1 内部控制面第二小批次 facade 验收；详细职责见文件查阅字典。
...
DEFAULT_MANIFEST = ROOT / "references/r1-control-plane-internal-maintainer-facade-batch-2-copy-first-apply.json"
...
ROUTE_DECISION = ROOT / "references/r1-next-safe-slice-after-internal-maintainer-facade-batch-preflight.json"
PREVIOUS_BATCH = ROOT / "references/r1-control-plane-internal-maintainer-facade-copy-first-apply.json"
EXPECTED_STATUS = "copy-first-internal-maintainer-facades-batch-2-old-anchors-retained"
...
def fail(message: str) -> None:
    raise SystemExit(f"[redcap-r1-control-plane-internal-maintainer-facade-batch-2-copy-first-apply-check] {message}")
...
def validate(manifest: dict[str, Any]) -> dict[str, Any]:
    if manifest.get("apply_id") != "redcap-r1-control-plane-internal-maintainer-facade-batch-2-copy-first-apply":
        fail("apply_id must be redcap-r1-control-plane-internal-maintainer-facade-batch-2-copy-first-apply")
    if manifest.get("status") != EXPECTED_STATUS:
        fail(f"status must be {EXPECTED_STATUS}")

    route = load_json(ROUTE_DECISION, "P4-22 route decision")
    preflight = load_json(PHYSICAL_PREFLIGHT, "R1 control-plane physical apply preflight")
    previous = load_json(PREVIOUS_BATCH, "P4-21 previous internal maintainer facade batch")
    ...
    previous_pairs = {
        (require_text(item.get("source"), "previous facades.source"), require_text(item.get("target_path"), "previous facades.target_path"))
        for item in require_list(previous.get("facades"), "previous facades", min_len=1)
    ...
    expected_truth = {
        "route_path": ("references/r1-next-safe-slice-after-internal-maintainer-facade-batch-preflight.json", sha256(ROUTE_DECISION)),
        ...
        "previous_batch_path": ("references/r1-control-plane-internal-maintainer-facade-copy-first-apply.json", sha256(PREVIOUS_BATCH)),
    }
    ...
    if source_truth.get("selected_backlog_item") != "P4-23":
        fail("source_truth.selected_backlog_item must be P4-23")
    ...
    if source_truth.get("previous_batch_count") != len(previous_pairs):
        fail("source_truth.previous_batch_count is stale")
    facades = require_list(manifest.get("facades"), "facades", min_len=1)
    if source_truth.get("selected_count") != len(facades):
        fail("source_truth.selected_count must match facades length")
    if route.get("result", {}).get("selected_next_backlog_item") != "P4-23":
        fail("P4-22 route decision must still point to P4-23")
    ...
    require_bool(boundary.get("second_batch_created"), True, "claim_boundary.second_batch_created")
    for key in [
    ...
    overlap = actual_pairs.intersection(previous_pairs)
    if overlap:
        fail(f"batch-2 facades overlap previous batch: {sorted(overlap)!r}")

    review = manifest.get("prism_review")
    if not isinstance(review, dict):
        fail("prism_review must be an object")
    if review.get("run_id") != "20260523-r1-control-plane-internal-maintainer-facade-batch-2-copy-first-apply":
        fail("prism_review.run_id must match P4-23 run")
    if review.get("verdict") != "consensus-implement-proposed-batch":
        fail("prism_review.verdict must be consensus-implement-proposed-batch")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate P4-23 internal-control-plane maintainer facade batch-2 copy-first apply.")
    parser.add_argument("--manifest", default=str(DEFAULT_MANIFEST))
    args = parser.parse_args()
    manifest = load_json(Path(args.manifest), "P4-23 internal-control-plane maintainer facade batch-2 copy-first apply manifest")
    summary = validate(manifest)
    print(
        "R1_CONTROL_PLANE_INTERNAL_MAINTAINER_FACADE_BATCH_2_COPY_FIRST_APPLY_OK "
