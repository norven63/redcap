...
7. **父任务线自动续跑**：子任务 receipt 生成后，若绑定的 backlog/current_focus 已指向下一项且未命中 §10 人工介入门，Cap 必须自动重锚并继续执行下一项；不得把“好的，请继续”这类机械授权转嫁给 Norven。只有三种情况允许停：无下一项、下一项命中人工保留决策/破坏性动作/外部账号凭据、或自动续跑检查失败且无法自修。
