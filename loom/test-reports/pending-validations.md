...
- **触发类型**：路由逻辑
- **验证要点**：当 Gemini / Copilot / Claude / Kimi reviewer 不可用时，stop-review 能 fallback 到 Codex CLI；程序化消费应读取 `--output-last-message`，stdout/stderr banner 或 warning 不得污染评审 payload；timeout 时必须杀掉 reviewer 进程组，不能遗留子进程。
