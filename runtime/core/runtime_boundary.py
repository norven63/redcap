...
def default_state_root(user_private_root: pathlib.Path) -> pathlib.Path:
...
def build_context(args: argparse.Namespace) -> dict[str, Any]:
    ...
    state_root = resolve_path(args.state_root or default_state_root(user_private_root))
    if args.evidence_root:
        evidence_root = resolve_path(args.evidence_root)
    elif project_workspace == runtime_root:
        evidence_root = runtime_root / "assets" / "evidence"
    else:
        evidence_root = state_root / "evidence" / project_hash(project_workspace)
...
def validate_context(context: dict[str, Any], *, require_task_file: bool = False) -> list[str]:
    ...
    if project_workspace != runtime_root and is_relative_to(evidence_root, project_workspace):
        failures.append("external project evidence_root must not default inside project_workspace")
    if project_workspace != runtime_root and is_relative_to(evidence_root, runtime_root):
        failures.append("external project evidence_root must not default inside RedCap runtime_root")
    if not is_relative_to(state_root, user_private_root) and project_workspace != runtime_root:
        failures.append("external project state_root should live under user_private_root")
    return failures
