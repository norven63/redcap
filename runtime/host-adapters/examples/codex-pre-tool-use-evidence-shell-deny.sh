...
run_probe() {
  ...
  local output
  output="$(
    python3 - "$command" "$PWD" <<'PY' | PWDDIR="$PWD" HOMEBREW="$PWD" REDCAP_CODEX_HOOK_EVIDENCE_DIR="$tmp_dir" python3 runtime/host-adapters/codex/codex-hook.py --event PreToolUse
import json
import sys
