...
- **根因**：① 需求描述在对话早期，随轮次增加被压缩截断 ② `.dev-task.md` 断点续传机制设计用于"跨会话恢复进度"，不解决"同会话内需求保真"问题 ③ Cap 执行时依赖记忆概括而非原始文本
- **经验规则**：① 用户提出 ≥3 个问题时，先执行 PM 确认门（CONTRIBUTING.md §10），澄清并锁定需求 ② 需求锁定为 PM 对话结束后的最终确认态（非初始原文——可能已经演进） ③ 执行每个 Q 前必须 re-read `.dev-task.md` 中的对应确认描述，不能依赖记忆
- **落地状态**：✅ CONTRIBUTING.md §10 Layer B 需求确认门
