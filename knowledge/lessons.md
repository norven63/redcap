...
### L-21: 长任务启动前必须做"目的备份"，防止上下文漂移遗忘初衷
...
- **经验规则**：① 开启任何预计超过 10 轮交互的长任务前，Dispatcher 必须在 session memory 或 state.yaml 中记录"本轮目的声明"（一句话：我要做什么、为什么、完成标准是什么）② 每完成一个子任务后，回读目的声明，确认是否存在遗漏的后续任务 ③ 如果用户打断提醒"你忘了 XXX"，必须反思：这个 XXX 是否应在目的声明中被预先记录
