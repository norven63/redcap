...
- [文件职责矩阵](#文件职责矩阵)
- [系统设计](#系统设计)
  - [总览：Dispatcher 事件循环](#总览dispatcher-事件循环)
  - [状态机](#状态机)
  - [通信协议](#通信协议)
  - [角色系统 + Prompt 模板](#角色系统--prompt-模板)
  - [可靠性工程](#可靠性工程)
    - [Hook 机制深度解读](#hook-机制深度解读)
    - [规则防退化](#规则防退化检查点重载)
    - [Pending Actions](#pending-actions待办持久化)
  - [经验库](#经验库)
- [设计哲学](#设计哲学)
  - [常见架构疑问](#常见架构疑问)
- [复刻指南：完整阅读路径](#复刻指南完整阅读路径)
...
Dispatcher 的核心是一个**事件循环**，每轮执行以下 10 步：
...
以下各节分别深入事件循环中的每个核心模块：状态机驱动流转（步骤 1-4）、通信协议解析返回（步骤 5e）、角色系统组装 Prompt（步骤 5a-5b）、可靠性工程保障执行（步骤 0, 5h-5i）、经验库沉淀复用。
...
RedCap 面对的核心挑战：**LLM 在长对话中的 attention 衰减导致指令遵从率下降**。三大防护机制分别解决三个不同维度的遗忘问题。
...
| 机制 | 解决什么 | 核心原理 | 详见 |
|----|---------|---------|------|
| **宿主 Hooks** | 关键动作遗漏 | 绕过 LLM，OS 级确定性执行 | Hook 机制 |
| **规则防退化** | 约束规则被压缩遗忘 | 检查点 `read_file` 强制刷新上下文 | 规则防退化 |
| **Pending Actions** | 待办动作被遗忘 | 原子写入 state.yaml，外部持久化 | Pending Actions |

#### Hook 机制深度解读

**核心洞察**：LLM 指令注入 ≠ 执行保证（详见 L-12），唯一 100% 确定性的是绕过 LLM 的宿主 Hooks。
...
#### Pending Actions（待办持久化）

**问题模型**

即使规则没退化（防退化机制保障），Dispatcher 仍可能在状态转移后遗忘后续动作。例如：更新了 `state=QA_PASS` 但忘了执行 git commit。这是**递归遗忘问题**——"防止遗忘的机制本身被遗忘"。
...
**防退化与 Pending Actions 的互补关系**

```
防退化（规则层）         Pending Actions（动作层）
┌─────────────────┐          ┌─────────────────────┐
│ 保护：静态约束   │          │ 保护：动态任务        │
