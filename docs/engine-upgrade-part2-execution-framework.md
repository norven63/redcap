...
### 5.1 初始方案：静态绑定（MVP 阶段）

> **重要前提**：当前环境中 Claude Code CLI 的底层模型为 **Kimi 2.5**（Moonshot AI，通过 SiliconFlow 代理接入），非 Claude 原生模型。以下路由基于两个底层模型的实际能力评估。

MVP 阶段采用固定的角色→Agent 映射，基于业内评测的能力匹配：

| 角色 | Agent | 底层模型 | 选择理由 |
|------|-------|---------|---------|
| 产品经理 | claude-code | Kimi 2.5 | 中文理解与需求分析能力强，长上下文支持好，适合需求对话 |
| 架构师 | gemini | Gemini 3 Pro | 系统设计推理能力突出，技术方案评估精准 |
| 程序员 | gemini | Gemini 3 Pro | 代码生成质量与工具使用能力在主流评测（SWE-bench、HumanEval）中领先 |
| 测试QA | claude-code | Kimi 2.5 | 长上下文审查能力适合代码 Review，中文测试报告输出细致 |

```yaml
agent_routing:
  product-manager: "claude-code"   # Kimi 2.5
  architect: "gemini"              # Gemini 3 Pro
  programmer: "gemini"             # Gemini 3 Pro
  qa: "claude-code"                # Kimi 2.5
```

具体的 Agent 分配可由用户在项目初始化时覆盖。上述默认配置遵循两个原则：
1. **核心工程角色（架构 + 编码）交给代码能力最强的模型**
2. **首尾角色（需求 + 验收）交给中文理解力最强的模型**，形成输入/输出的一致性
...
路由因素：
  · 任务类型（设计 vs 编码 vs 测试）
  · 上下文连续性（同一 step 内优先复用同一 Agent）
  · Agent 可用性（某个 Agent 的 API 是否可达）
  · 历史表现（可选：记录成功率做偏好学习）
```

动态路由为远期演进方向，不在本次升级范围内。
...
Phase 2: 角色手册改造
  ...
  · 适配变更：
    - 移除"角色切换"相关的 Prompt 指令（不再需要同一 AI 切换角色）
    - 新增"交付物输入/输出路径"的明确指引
    - 新增"状态报告格式"（__redcap_status JSON 块）的输出要求
    - 保持业务逻辑不变（验收标准、质量门禁等）

Phase 3: Dispatcher 协议实现
...
Phase 4: 端到端验证
  · 用一个简单的测试项目跑通完整流程
  · 验证所有状态转移路径（正向流转、回退、升级、暂停恢复）
  · 验证 Session 持久化与恢复
  · 确认交付物在角色间的传递完整性

Phase 5: 合并与文档收尾
  · 更新 SKILL.md 根文档
  · 更新各角色 templates/ 下的文档（如需）
  · 合并到 master
```
