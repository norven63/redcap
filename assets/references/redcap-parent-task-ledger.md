...
当前已完成的是若干控制面、路线图子任务、package readiness、Prism quorum 复验、shared-knowledge 远端模板绑定、首次启动身份/通知策略链路、父任务 completed child 的 runtime receipt 内容对应强门、P3-1 检索升级阈值策略、P4-1 的历史资产 copy-first、delete-last 与 private canonical switch、P4-3 的 clean workspace / cross-machine-style 安装 E2E、P4-2a 发布前产品架构审判、P4-2f 信息架构与运行时产物治理、P4-2g npm 白名单预检与结构重构任务树重锚定、P4-2b runtime/project/user 边界与 CLI workspace context、P4-2c CLI doctor/debug/trace/error/help 产品面加固、P4-2d public package identity/license/surface 准备、P4-2e public redcap-arsenal 内容策略与 claim 边界、P4-2h 首批 RedCap Forge 公共晋升、P4-2h-0 历史资产公共蒸馏 preflight、P4-2h-1 AI Era agent-reading 资料吸收、P4-2h-2 LLM-wiki 资产分层评估与需求登记、P4-2h-3 LLM-wiki-lite 最小私有语义记忆生命周期、P4-2i runtime 最小兼容物理布局、P4-2j 发布前最终收束审判、P4-2k 发布前非发布产品化治理收束、P2-6 Copilot protected fallback 策略收紧、P2-7 中途架构与任务树一致性审计、P2-8 Prism runs 生命周期状态面与清理边界加固、P2-9 飞书完成通知单出口收敛、P2-10 飞书节点汇报人类可读化、P2-14 飞书回复安全收件箱，以及 P2-16 Codex lifecycle hooks candidate 接线。P2-16 现在已经完成本机 Codex CLI live marker E2E，证明 `codex exec` 会物理触发 SessionStart / Stop；但这仍不等于 Codex.app interactive 已验证，也不等于 full host parity 或完整 reply-veto。P4-2k 完成的是正式 release task 前的工程收束：npm 候选包面当前为 184 个文件的 alpha readiness surface，并已补 runtime import map、public release handoff、RASG-020 公开/维护者契约分层与 RASG-023 计划型后续登记门；它不是 npm publish，也不选择 license 或开启发布开关。P4-2j 完成的是最新 release posture 收束审判：安全预检和本地 readiness 通过，但正式公开发布仍被许可证、发布开关/凭据、registry 权限等人工边界阻塞；它不是 npm publish。P4-2h/RASG-028 当前完成的是 8 条 reviewed-substantive 公共方法论/经验样本、索引和远端一致性，其中包含第二批 RedCap Forge append-only 公共扩容，不等于全部历史资产公共迁移或成熟公共知识库；P4-2h-0 只证明 RedCap Forge 预检和晋升前边界仍可审计；P4-2h-1 只把 `ai-engineer.md` 与 `llm-wiki.md` 的可用设计吸收到 RedCap 的长期记忆和工程纪律契约，不等于启用完整 LLM Wiki、RAG/GraphRAG 或公共库直写；P4-2h-2 只完成 RedCap 资产是否适合进入 LLM-wiki-lite 语义记忆层的分层判断；P4-2h-3 只实现私有、非权威、带 source anchor、可过期检测、Forge 晋升受控的最小语义记忆层，不等于完整 LLM-wiki 产品、后台自动蒸馏、RAG/GraphRAG、向量库或公共写回；P4-2i 只证明 package-visible runtime root 与兼容 wrappers 已存在，不等于完整执行层拆分、工具树迁移或 public-release-ready；P2-6 只把 Copilot CLI 从普通自动路径降级为 Claude Code 与 Kimi 均不可用时才允许调用的保护性 fallback，不等于禁用用户手动 Copilot；P2-7 只完成文件头/字典一致性、LLM-wiki-lite 边界可见性和中途任务树对账，不等于 public release ready；P2-8 只把 `prism/runs` 残留提示改成可执行、可审查且明确 `--apply` 需显式批准的状态面，不等于已经物理清理运行证据；P2-9 只把 Layer B 正常完成的飞书 node-report 收敛为 closeout runtime receipt 后单出口，不等于飞书平台侧延迟归零或所有宿主原生通知能力都可控；P2-10 只把飞书正文改成人类可读 compact 汇报，不等于重构终端状态面或改变通知发送策略；P2-14 只让飞书回复进入安全收件箱，不等于实时远程控制 RedCap。P4-2 正式公开发布仍未完成，不能被简化为“只差 npm publish”；它仍被 formal release task 的人工许可证、发布开关/凭据和 registry 权限阻塞。

P4-2o 完成的是 formal release R1 前置分类：当前包候选面为 199 个文件，workspace-state 已证明不进入候选包，但 `internal-control-plane`、`prism-layer-and-evidence`、`internal-layer-a` 仍是 release blockers；它不是 R1 cleanup closed、不是 public-release-ready，也不执行物理迁移、删除、license 或发布开关。
...
| P4-2s | R1 控制面物理拆分干跑清单 | completed | 已为 compass/references 的 package-visible control-plane 候选生成未来目标分层、别名/回滚计划和 dry-run 验收；不移动目录、不关闭 blocker、不发布。 |
...
| redcap-arsenal 本地实体仓库与 Norven 命名空间 | 已建立 `/Users/norven/.claude/skills/redcap-arsenal` 耐久本地 Git 工作区，模板/实体/远端均含 `users/Norven/.gitkeep` | 报告 `2026-04-27-redcap-arsenal-local-worktree-and-user-namespace.md`；外部公共库初始化 commit `2e3b954` | 不等于首批 reviewed-substantive 公共条目；该边界由 P4-2h 证明 |
...
| Runtime receipt evidence correspondence hardening | 父任务 completed child 聚合已从 `receipt_glob` 字符串形态升级为真实 runtime receipt 内容核对，覆盖 task_id、report_path、completed、promise_pending、acceptance_status 与 git head | 报告 `2026-04-28-runtime-receipt-evidence-correspondence-hardening.md`；产物：`references/parent-receipt-aggregation-policy.json`、`redcap-parent-receipt-aggregation-check.py` | 不等于父任务整体 complete；后续 release/apply/E2E 边界仍需单独任务 |
| Retrieval escalation threshold policy | P3-1 已将 GraphRAG / 向量检索研究从口头 deferred 升级为机器可审计阈值策略；当前路线保持 `catalog + rg + metadata`，只有规模或失败观测跨阈值才允许另开 FTS/RAG/GraphRAG 评估任务 | 报告 `2026-04-28-retrieval-escalation-threshold-policy.md`；产物：`references/retrieval-escalation-policy.json`、`redcap-retrieval-escalation-check.py` | 不等于 RedCap 已启用 GraphRAG，也不等于公共库已有大量真实条目 |
...
| P1-4 | user inserted follow-up | redcap-arsenal 本地实体仓库与 Norven 命名空间 | completed | P1 | 已建立外部本地 worktree、推送 `users/Norven/.gitkeep`，并补模板/实体/远端三层口径和验收 | report: `2026-04-27-redcap-arsenal-local-worktree-and-user-namespace.md`；initial remote head: `2e3b954338a4c397d299da88f460c6edf5a312d6` |
...
| P4-1a | parent boundary | 历史资产 main-tree copy-first apply | completed | P1 | 54 个 copy-first target 已创建到私有 `private-archive/redcap-knowledge/**`，resolver 显示 `applied-copy-present`，旧 `compass/docs/**` 仍是 canonical anchor | 不等于 delete-last / canonical switch / public export |
| P4-1 | parent boundary | 历史资产迁移 delete-last / canonical switch | completed | P1 | 54 个旧 `compass/docs/**` 历史锚点已按 preflight + Prism review + apply receipt 退休，catalog 已刷新，resolver 已切到 `private-archive/redcap-knowledge/**` canonical | 不等于 public export；私有知识库仍不得直接发布 |
...
| P4-2a | parent boundary | 发布前产品架构审判 | completed | P0-before-public-release | 已建立 pre-release product architecture gate；P4-2i 后当前仍有 2 个人工 release blocker（发布开关/凭据、license）、2 个 should-fix、1 个 deferred；历史 Kimi + Claude Code 复审通过 | 完成的是“审判和拦截机制”，不等于 release blocker 已修完，也不等于 public release ready |
...
| P4-2e | P4-2a finding | public redcap-arsenal 内容策略与 claim 边界 | completed | P1-before-broad-marketing | 已锁定 template-only 与 reviewed-substantive 两种阶段的允许/禁止 claim，接入机器检查、Prism 复核和外部公共库 README 口径修正 | 不得把首批样本说成成熟公共知识库；默认不阻塞 CLI release engineering |
| P4-2f | user architecture review | RedCap 信息架构与运行时产物治理 | completed | P0-before-public-release | 已治理 report / private knowledge / runtime evidence / RedCap Forge / redcap-arsenal 的目录边界、生命周期和人类可读汇报规则 | 不等于 public release；公共库实质条目由 P4-2h 首批样本单独证明；报告 `2026-05-04-redcap-information-architecture-and-artifact-governance.md` |
...
| P4-2h | user release-path review | 历史资产公共蒸馏与 RedCap Forge export triage | completed | P1-before-broad-marketing | 已通过 RedCap Forge 风格流程把 3 条通用方法论/治理经验以 append-only 方式写入外部 redcap-arsenal，并生成索引、远端 head 和 claim boundary 证据 | 不等于全部历史私有资产公共迁移；不得把私有历史报告、identity、runtime evidence 直接写入 redcap-arsenal |
| P4-2h-0 | Prism route review | 历史资产公共蒸馏 preflight | completed | P1-before-broad-marketing | 已建立 RedCap Forge dry-run 预检强门，并在首批晋升后继续验证 raw 私有资产不直出、不删除、不发布 | 不等于批量公共迁移；不等于 npm release ready |
...
| P4-2i | P4-2a finding | runtime 最小兼容物理布局 | completed | P0-before-public-release | 已新增 package-visible `runtime/redcap-core/**` 与 `runtime/host-adapters/**` 最小布局，runtime wrappers 委托根入口，package/checker 事实已同步到 `minimum-compatible-layout-exists` | 不等于完整执行层拆分、`compass/tools`/`prism/tools` 迁移、npm publish 或 public release ready |
| P4-2j | parent release posture | 发布前最终收束审判 | completed | P0-before-public-release | 已按最新 HEAD 重新复核 package safety、manifest、public surface、pre-release 架构、结构任务树和 clean workspace E2E；pre-release check 当前仍明确给出 2 个 release blocker（发布开关/凭据、license）。结论是本地 readiness 成立，但正式公开发布仍需 Norven 决定许可证、发布开关、npm 凭据/registry 权限和发布窗口 | 不等于 npm publish；不等于 public-release-ready；不自动修改 `private`、`publish_allowed` 或 `license` |
...
| P4-2s | formal release R1 dry-run | R1 控制面物理拆分干跑清单 | completed | P0-before-release-task | 已建立 internal-control-plane 的 future target map、alias/rollback plan 和 dry-run coverage proof | 不等于控制面已物理拆分；不等于 release blocker resolved 或 public-release-ready |
...
| P2-7 | user inserted follow-up | 中途架构与任务树一致性审计 | completed | P2 | 已把关键脚本中文短头/字典反链、LLM-wiki-lite 边界可见性、历史/中插/计划任务对账纳入机器检查和报告闭环 | report: `private-archive/redcap-knowledge/task-reports/2026-05-07-redcap-midcourse-architecture-task-tree-review.md`；Prism: `20260507-midcourse-architecture-task-tree-review` resource-limited-pass；不等于 public release ready |
...
| P2-16 | user inserted Codex update | Codex lifecycle hooks candidate 接线 | completed | P2 | 已接入 `.codex/config.toml` / `.codex/hooks.json`、SessionStart / PreToolUse / Stop wrapper、readiness/status/知识画像和 acceptance；本机 Codex CLI live marker E2E 已通过 | report: `2026-05-09-codex-lifecycle-hooks-candidate.md`；evidence: `references/codex-live-marker-e2e.json`；Codex.app interactive 仍未单独证明，不等于 full host parity 或完整 reply-veto |
...
12. `P2-6`：Copilot protected fallback 策略收紧。（已完成；Copilot 自动路径只在 Claude Code 与 Kimi 都不可用时降级调用）
13. `P3-2`：runtime receipt evidence correspondence hardening。（已完成）
14. `P3-1`：GraphRAG / 向量检索阈值研究。（已完成阈值策略；当前不启用 GraphRAG）
15. `P4-1a`：历史资产 main-tree copy-first apply。（已完成）
16. `P4-1`：历史资产 delete-last / canonical switch。（已完成 private canonical switch；public export 不在本项范围）
17. `P4-2a`：发布前产品架构审判。（已完成；结论是 public release 仍不 ready）
18. `P4-2f`：RedCap 信息架构与运行时产物治理。（已完成；report/knowledge/runtime/arsenal/Forge 边界已机器化）
19. `P4-2g`：npm 白名单预检与结构重构任务树重锚定。（已完成；先拍片再手术）
20. `P4-2b`：runtime/project/user boundary 物理拆分与 CLI workspace context。（已完成最小 workspace context 闭环；不等于 runtime layout 大迁移）
21. `P4-2c`：CLI doctor/debug/trace/error/help 产品面加固。（已完成最小 public diagnostic surface；不等于 release-ready）
22. `P4-2d`：public package identity/license/surface 准备。（已完成 readiness 准备；不等于发布）
23. `P4-2e`：public redcap-arsenal 内容策略与 claim 边界。（已完成；P1，不默认阻塞 release engineering）
24. `P4-2h-0`：历史资产公共蒸馏 preflight。（已完成；首批晋升后继续作为 raw 私有资产不直出的边界检查）
25. `P4-2h-1`：AI Era agent-reading 吸收与长期记忆对齐。（已完成契约吸收；不启用完整 LLM Wiki/RAG/公共直写）
...
35. `P2-14`：飞书回复安全收件箱。（已完成；飞书回复可被软扫描成待处理入口，但不自动执行）
36. `P2-16`：Codex lifecycle hooks candidate 接线。（已完成最小接线；真实 trusted Codex marker E2E 前仍 degraded）
37. `P4-2h`：历史资产公共蒸馏与 RedCap Forge export triage。（已完成首批 reviewed-substantive 公共样本，独立于 npm 发布关键路径）
38. `P4-2i`：runtime 最小兼容物理布局。（已完成 package-visible runtime root 与 wrappers；完整工具树迁移仍不在本项范围）
39. `P4-2j`：发布前最终收束审判。（已完成最新 release posture 复核；本地 readiness 成立，但正式发布仍需人工 release task）
...
48. `P4-2s`：R1 控制面物理拆分干跑清单。（completed；只生成 future map / alias / rollback / coverage proof，不做物理迁移）
...
- 不可声明 RedCap 已经 public-release-ready；P4-2k 只完成正式 release task 前的非发布工程收束，发布开关/凭据与 license 仍是两个人工 release blocker。
...
- 不可声明 `redcap-arsenal` 已经成熟或完成历史知识迁移；当前只有 8 条 reviewed-substantive 公共方法论/经验样本、索引和 `users/Norven/` 命名空间。
...
- 不可声明 P4-2i 的最小 runtime 布局等同于完整执行层拆分；`compass/tools`、`prism/tools` 和宿主导入仍保留原路径。
...
- 不可声明 P2-14 让飞书回复变成实时远程命令；它只把回复收入安全待处理入口，后续仍必须走任务治理流程。
- 不可声明 P2-16 让 Codex 已经 full host parity；它已证明本机 Codex CLI `exec` 会物理触发 SessionStart / Stop marker，但 Codex.app interactive 与 reply-veto 仍未被证明。
