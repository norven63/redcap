...
Each provider has one session handle for the task:

- Kimi: the resume id printed by Kimi after the first task call.
- Claude Code: the explicit `--session-id` used for the first task call.
...
```bash
runtime/prism/bin/prism-dispatch --provider kimi --manifest assets/evidence/prism/<task-id>/session.json --request assets/evidence/prism/<task-id>/request.json --review-out assets/evidence/prism/<task-id>/kimi.review.json
```
...
- reject: the concern is invalid, with concrete evidence; this requires one
  bounded same-provider rebuttal request generated by `prism rebuttal-request`,
  and the follow-up review must return `pass`.
...
Provider disagreement is handled in bounded rounds:

1. Round 1: both providers review the same request.
2. If their conclusions conflict materially, RedCap sends the conflict summary
   back to both providers in their existing task sessions.
3. Each provider may revise, hold, or narrow its conclusion.
4. The default maximum is 3 rounds per provider.
5. `prism-dispatch` must pass `session-check` before every follow-up provider
   call.
6. If conflict remains after the round cap, RedCap must either make a documented
...
- both providers reach the same material verdict and minimum fix;
- one provider's concern is accepted and implemented or reflected in the claim;
- RedCap rejects a provider concern with evidence and records the rejection;
