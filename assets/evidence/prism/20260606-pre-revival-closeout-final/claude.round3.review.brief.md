...

{
  ...
  "changed_reality": [
    ...
    "The dispatcher now has --task-total-timeout-seconds, computes remaining task budget from session created_at, prevents provider launch when the task budget is exhausted, persists task budget fields in raw metadata, and emits a bounded Chinese timeout report.",
    ...
    "A separate runtime/prism/bin/prism-dispatch-timeout-e2e script now tests task-total budget exhaustion outside prism-dispatch --self-check and verifies raw metadata plus Chinese timeout reporting.",
  ...
  "evidence": [
    ...
    },
    {
      ...
      "summary": "Independent integration test confirms task-total exhaustion refuses provider launch and emits verified metadata plus Chinese report."
    },
    {
      "kind": "command",
