...

{
  ...
  "changed_reality": [
    ...
    "A real-provider timeout trace now launches Kimi through prism-dispatch with a 2-second task-total budget; the dispatcher times out the live provider call, writes verified raw metadata, emits a Chinese timeout report, passes that report through the human-output checker, and leaves no new provider process after the trace.",
    "A second real-provider timeout trace now covers Claude Code with the same 2-second task-total budget and the same metadata/report/process cleanup checks.",
    "Stop Hook now lints the final assistant message with the human-facing output policy before turn closeout; the Codex hook self-check proves an English-only final reply is blocked.",
    "The two currently open task facts are identified: full-360-old-redcap-scan is the next-phase scan item, and pre-revival-zero-tail-infra-batch is the parent batch awaiting Prism resolution; timeout governance and human-output policy are already verified child facts, not hidden open tasks.",
    "A bounded archaeology smoke test loads the existing old RedCap long-task-context-defense extraction from assets/archaeology/extractions/long-task-context-defense-v1.json, parses its schema, and confirms one bounded source plus six portable designs without bulk-reading the old repository."
  ],
  "requested_review": [
  ...
  "evidence": [
    ...
    {
      "kind": "behavior-trace",
      "reference": "assets/evidence/prism/20260606-real-provider-timeout-trace/",
      "summary": "Real Kimi dispatch with task-total 2s exits through provider-timeout, raw metadata verification exits 0, timeout-report exits 0, generated Chinese report lint exits 0, and process diff shows no new provider process after the trace."
    },
    {
      "kind": "behavior-trace",
      "reference": "assets/evidence/prism/20260606-real-provider-timeout-trace-claude/",
      "summary": "Real Claude Code dispatch with task-total 2s exits through provider-timeout, raw metadata verification exits 0, timeout-report exits 0, generated Chinese report lint exits 0, and process diff shows no new provider process after the trace."
    },
    {
      "kind": "command",
      "reference": "python3 runtime/host-adapters/codex/codex-hook.py --self-check",
      "summary": "Codex Stop Hook self-check exits 0 and includes a fixture proving an English-only final assistant reply is blocked by the human-output guard."
    },
    {
      "kind": "command",
      "reference": "runtime/bin/redcap task-facts summary",
      "summary": "The open tasks are explicitly categorized: full-360-old-redcap-scan is the next phase; pre-revival-zero-tail-infra-batch is the parent batch pending Prism resolution, not a hidden child infrastructure item."
    },
    {
      "kind": "command",
      "reference": "runtime/bin/redcap archaeology check && runtime/bin/redcap archaeology self-check",
      "summary": "Bounded archaeology shard index and fixtures validate without reading the old RedCap repository in bulk."
    },
    {
      "kind": "command",
      "reference": "parse assets/archaeology/extractions/long-task-context-defense-v1.json",
      "summary": "The long-task context-defense extraction loads successfully with schema old-redcap-long-task-context-defense-extraction, one bounded source, and six portable designs."
    }
  ],
  "review_mode": "implementation_review",
  "risk_level": "high",
